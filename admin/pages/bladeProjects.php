...
if ( isset($_POST["title"]) ){
	...
	echo "<script>window.location='?p=Projects'</script>";die();
}
...
<tbody>
<?php
$sql = "SELECT p.*, u.username, c.name as clientName
		FROM `project` as p
		JOIN `user` as u
		ON p.userId = u.id
        JOIN `client` as c
...
<?php }if ( $status[$i] != '0' ){  ?>
<a href="?p=Projects<?php echo "&return=" . $row["id"] ?>" style="margin:3px"><i class="fa fa-refresh"></i></a>
<?php }if ( $status[$i] != '2' ){  ?>
<a href="?p=Projects<?php echo "&delete=" . $row["id"] ?>" style="margin:3px"><i class="fa fa-trash"></i></a>
<?php } ?>
</td>
</tr>
<?php
