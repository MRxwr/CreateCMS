...
<div class="pull-left">
<h6 class="panel-title txt-dark">Add New Employee</h6>
</div>
...
<?php
$status 		= array('0','1');
$arrayOfTitles 	= array('Current Employees','Lost Employees');
...
<tbody>
<?php
$sql = "SELECT e.*, u.username as user
		FROM `employee` as e
		JOIN `user` as u
		ON u.id = e.userId
		WHERE
		e.status LIKE '".$status[$i]."'
		";
